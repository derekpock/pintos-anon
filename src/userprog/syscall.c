...
unmap (struct mapping *m)
{
  ...
  //Look for changed pages and write out changes.
  for(int i = 0; i < m->page_cnt; i++){
    if(pagedir_is_dirty(thread_current()->pagedir, (m->base + (i * PGSIZE)))){
      lock_acquire(&fs_lock);
      file_write_at(m->file, (m->base + (i * PGSIZE)), (PGSIZE * m->page_cnt), (PGSIZE * i));
      lock_release(&fs_lock);
    }
  }
...
{
  struct mapping *map = lookup_mapping(mapping); //look up map to be unmapped
  unmap(map);                                    //unmap map
  /*struct list_elem *e;
  for (e = list_begin (&thread_current()->mappings); e != list_end (&thread_current()->mappings);
       e = list_next (e))
  {
    struct mapping *m = list_entry (e, struct mapping, elem);
    if(m->handle == mapping) {
